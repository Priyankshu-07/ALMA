...
def estimate_fetal_weight(
    ...
    if hc_cm and ac_cm and fl_cm:
        ...
        if gestational_age is not None and fl_cm < 4.0:
            efw = _lookup_efw_from_ga(gestational_age)
            logger.info(f"[hadlock] FL too small for formula — using GA reference → EFW={efw:.1f}g")
            return round(efw, 2)
        else:
            log_efw = 0.9119 + 0.4162 * fl_cm
            formula_used = "FL only (approximate)"
    elif hc_cm:
        if gestational_age is not None and gestational_age < 20:
            efw = _lookup_efw_from_ga(gestational_age)
            logger.info(f"[hadlock] Early pregnancy HC — using GA reference → EFW={efw:.1f}g")
            return round(efw, 2)
        else:
