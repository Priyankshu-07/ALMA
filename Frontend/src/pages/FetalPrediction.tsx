...
const resultConfig: Record<FetalResult, { color: string; bg: string; icon: typeof CheckCircle; desc: string }> = {
  Normal: { color: "text-success", bg: "bg-success/10", icon: CheckCircle, desc: "Fetal health appears normal. Continue routine monitoring." },
  Suspect: { color: "text-warning", bg: "bg-warning/10", icon: AlertTriangle, desc: "Some indicators require attention. Further evaluation recommended." },
  Pathological: { color: "text-destructive", bg: "bg-destructive/10", icon: XCircle, desc: "Critical indicators detected. Immediate medical attention advised." },
};
...
export default function FetalPrediction() {
  ...
  const handleAnalyze = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch("http://localhost:8000/predict-fhr", {
        method: "POST",
      ...
      setResult(data.result || "Normal");
    } catch {
      const severe = Number(form.severeDecelerations) || 0;
      setResult(severe > 0.001 ? "Pathological" : Number(form.accelerations) < 0.001 ? "Suspect" : "Normal");
        ...
        <CardHeader>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">Enter CTG parameters for analysis</p>
            </div>
          </div>
        </CardHeader>
        <CardContent>
          ...
          <Button className="w-full mt-6" size="lg" onClick={handleAnalyze} disabled={loading}>
      ...
      <AnimatePresence>
        {config && result && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            <Card className={`border-2 ${result === "Pathological" ? "border-destructive/30" : result === "Suspect" ? "border-warning/30" : "border-success/30"}`}>
              <CardContent className="p-6 flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className={`text-2xl font-display font-bold ${config.color}`}>{result}</p>
